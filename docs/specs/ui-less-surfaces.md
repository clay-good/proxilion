...
- `proxilion_oauth_callback_total{idp,result}` — emitted at the end of `google_callback` (idp="google"); same `result` classifier. The `bridge_callback` (federation_token → PCA_0) is intentionally **not** mirrored here today — it's a thin pass-through and the federation IdP label would require parsing the JWT iss claim per call. When federation-bridge-bin ships (§0.4 follow-up), the bridge callback joins this metric with `idp="okta|azure|google_workspace|oidc"`.
