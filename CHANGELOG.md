...
- **Declared MSRV corrected from 1.85 to 1.88, and the lints that unlocked.**
  ...
  `rust-version = "1.88"`, so the workspace had not resolved on 1.85–1.87 for
  some time — the stale floor bought a confusing resolver error instead of a
  clear "requires rustc 1.88". Clippy gates several lints on the declared MSRV,
  so telling it the truth also enabled `collapsible_if` for let-chains and
  `manual_is_multiple_of` (both stabilized by 1.88): 36 sites across
  `crates/cli` and `crates/proxy` now read as single `if a && let Some(b) = c`
  chains instead of nested `if`s. Every rewrite was generated by
  `cargo clippy --fix` and is semantically identical (the lint only fires where
  the inner `if` has no `else`, so short-circuit behavior is unchanged); the
  full suite is green. The Dockerfile's builder was already `rust:1.88` — only
  its comment claimed 1.85.
