...
- [crates/proxy/src/oauth/](../../crates/proxy/src/oauth/) — four routes (`/oauth/google/authorize`, `/oauth/bridge/callback`, `/oauth/google/callback`, `POST /oauth/google/token`), `OAuthError → Response` mapping that never leaks detail to the body, server-side scope intersection (Google scope ⨯ PCA_0 ops).
...
2. ~~**Federation token signature verification is stubbed** — `oauth::bridge::validate_federation_token` decodes payload only.~~ **Resolved 2026-09-05 (production-readiness.md PR-1, Approach A).** `/oauth/bridge/callback` now takes an IdP `id_token`, verifies its signature against the issuer's JWKS with the algorithm pinned server-side ([`oauth::federation`](../../crates/proxy/src/oauth/federation.rs) composing [`oauth::idp_verify`](../../crates/proxy/src/oauth/idp_verify.rs) + [`oauth::jwks`](../../crates/proxy/src/oauth/jwks.rs)), and mints PCA_0 **in-process** from the verified identity via Trust Plane `POST /v1/pca/issue` — so the forgeable bridge→proxy token no longer exists on that path, which is why no bridge *service* was needed after all. Configure with `PROXILION_IDP_ISSUER` / `_AUDIENCE` / `_JWKS_URI` / `_ALGORITHMS`. The payload-only `validate_federation_token` path survives only for dev/CI/smoke behind `PROXILION_INSECURE_BRIDGE_STUB` (default on) and a protected `PROXILION_ENV` refuses to boot while it is active or while the verified path is unconfigured.
3. **No wiremock'd Google integration test yet.** `wiremock` is already in `[dev-dependencies]`; the test harness needs a running postgres, which we'd want via `sqlx::testcontainers` or a CI-only `services:` block. Tracked as a follow-up. **Partially addressed (2026-06-11):** an opt-in DB-backed integration layer now exists — [crates/proxy/src/test_support.rs](../../crates/proxy/src/test_support.rs) yields a migrated pool when `PROXILION_TEST_DATABASE_URL` is set (and skips otherwise, so the default suite and CI stay hermetic/green). It drives real handlers against real SQL: the `notifier_public` approval landing (asserting the email link consumes its single-use token only on **POST**, never on a prefetch GET), the `killswitch` dry-run (asserting the preview `count(*)` matches the real revoke exactly with no state change), and the `blocked` queue (list filters + the auto-expire-on-list side effect + `show`). **Now CI-enforced (2026-06-11):** the `integration` job in [.github/workflows/ci.yml](../../.github/workflows/ci.yml) provisions a `postgres:16-alpine` service and runs `cargo test -p proxy db_backed` on every push, so these run for real (not skipped) in CI. The Google/Trust-Plane wire-level scenarios (which also need wiremock + seeded `pca_cache` + a successor PoC) remain the next slice.
...
- [crates/proxy/src/session.rs](../../crates/proxy/src/session.rs) — `SessionContext` (Debug redacts the Google token) and `SessionCtx` extractor pulling `Arc<SessionContext>` from request extensions.
- [crates/proxy/src/pic/cat_key.rs](../../crates/proxy/src/pic/cat_key.rs) — `CatKeyRegistry`: lazy fetch + cache of the Trust Plane CAT verifying key via `GET /v1/federation/info`.
- [crates/proxy/src/auth_middleware.rs](../../crates/proxy/src/auth_middleware.rs) — `auth_middleware` does: header → format check (`Bearer::parse`) → DB join (`agent_bearers ⨯ google_tokens ⨯ oauth_sessions`) → revocation check → decrypt → refresh-if-near-expiry (coalesced via moka-cached per-bearer `tokio::sync::Mutex`) → load PCA_1 from `pca_cache` → verify CAT signature → build `SessionContext`. Every failure path returns a fixed `401 unauthorized` body; the cause goes to `tracing::warn!` and `metrics::counter!` only.
...
- Mounted via a small protected route in [server.rs](../../crates/proxy/src/server.rs) — `GET /internal/whoami` returns a redacted view of `SessionContext`. Exercises the middleware end-to-end; real adapters land in §1.3.
...
3. ~~**Concurrent-refresh test is structural, not integration.**~~ **Landed (2026-06-11).** [crates/proxy/src/auth_middleware.rs](../../crates/proxy/src/auth_middleware.rs) `db_backed_refresh_coalesces_50_concurrent_into_one_google_call` drives 50 concurrent `refresh_with_coalescing` calls (same bearer hash, an EXPIRED seeded `google_tokens` row with encrypted access + refresh tokens) against a **wiremock'd Google token endpoint** and asserts — empirically, via `received_requests()` — that Google is hit **exactly once** and all 50 callers get the fresh token. This pins the per-hash `Arc<Mutex>` + post-lock DB re-read coalescing under real concurrency (the first refresher does the POST and persists; the rest re-read the freshened expiry under the lock and coalesce). Runs in the CI `integration` job; skips without `PROXILION_TEST_DATABASE_URL`.
...
- [crates/proxy/src/adapters/google_drive.rs](../../crates/proxy/src/adapters/google_drive.rs) — three routes (`/google/drive/v3/files`, `/files/{id}`, `/files/{id}/export`) sharing `proxy_request`. Each call: builds `RequestContext` → evaluates Layer B → enforces pre-request decision → mints PCA_2 successor (Layer A) with narrowed ops from `outcome.required_ops.resolve(&ctx)` → caches PCA_2 → fetches Google with the decrypted session token → reads ≤ 10MB → applies the read filter → publishes an `ActionEvent` → returns with `x-proxilion-{request-id,pca-id,policy}` response headers.
- [crates/proxy/src/adapters/state.rs](../../crates/proxy/src/adapters/state.rs) — `AdapterState` carries policy engine + executor + upstream client + action stream + a `google_api_base` override for wiremock'd tests + customer domain.
- [server.rs](../../crates/proxy/src/server.rs) wires the adapter router behind the same `auth_middleware` as `/internal/whoami`; gated on the same env prereqs as the OAuth routes (DB + encryption key + Google creds).
...
**Spec deviations to flag:**
1. ~~**End-to-end wiremock'd Google test deferred.**~~ **Landed (2026-06-11) for the read-filter scenario.** [crates/proxy/src/adapters/google_drive.rs](../../crates/proxy/src/adapters/google_drive.rs) `db_backed_drive_get_audit_mode_read_filter_quarantines_injection` drives the full `proxy_request` path end-to-end: policy eval → PIC mint attempt against a **wiremock'd Trust Plane** (which returns 422, so `audit` mode falls back to the predecessor PCA — no real crypto needed) → upstream GET to a **wiremock'd Google** (via the `google_api_base` override) → read-filter quarantine → filtered response. It asserts the injection pattern (`ignore previous instructions`) is replaced by `read_filter::MARKER` while surrounding text passes through ("read filter triggers → marker present"). Seeds the `pca_cache` predecessor row; runs in the CI `integration` job (postgres service) and skips locally without `PROXILION_TEST_DATABASE_URL`. **The two remaining wire-level scenarios also landed (2026-06-11):** `db_backed_drive_get_runtime_gate_forced_ops_mismatch_is_blocked_403` (runtime-gate mode — the same Trust Plane 422 surfaces as a `PicInvariantViolation` → 403, the upstream is never reached, and a `blocked_actions` row with `layer='pic_invariant'` is persisted — the "prevention by construction" guarantee) and, in [google_gmail.rs](../../crates/proxy/src/adapters/google_gmail.rs), `db_backed_gmail_send_external_recipient_is_blocked_403` (the flagship §9 Layer-B gate — an external recipient blocks at Layer B → `PolicyBlocked` → 403 + a `layer='policy'` blocked row, no Trust Plane / Google contacted). The shared scaffolding lives in [test_support.rs](../../crates/proxy/src/test_support.rs) (`adapter_state`, `mock_session`, `mock_trust_plane_reject`/`mock_trust_plane_issue`). **The adapter `proxy_request` template is now integration-covered across its full branch matrix (2026-06-11):** the happy path (`mock_trust_plane_issue` returns a valid successor → the PCA_2 is cached at `hop=2` with the leaf as predecessor and a clean body passes through), the read-filter `block_request` action (full-body quarantine → `ReadFilterBlocked` 403 + `layer='read_filter'` row, distinct from the `replace_with_marker` path), the runtime-gate mint-refusal (403), and the Layer-B Gmail block (403). The empirical concurrent-refresh "50 → 1 Google call" count is also covered now (§1.1 deviation 3 — `db_backed_refresh_coalesces_50_concurrent_into_one_google_call`). The only paths still on manual verification are the live Google/Calendar **wire formats** against a real Workspace account (§2.1) — i.e. that our request shaping matches Google's actual API, which a wiremock can't prove.
...
- [migrations/0002_quarantine_blocked.sql](../../migrations/0002_quarantine_blocked.sql) — `quarantined_payloads` (per-match audit row with pattern + 200-char snippet), `blocked_actions` (Layer-A `pic_invariant` and Layer-B `policy` / `read_filter` rejections), and `pca_verification_results` (cache slot for §1.5; persistent verification record for §1.6).
- [crates/proxy/src/adapters/read_filter.rs](../../crates/proxy/src/adapters/read_filter.rs) rewritten: every pattern (literal *or* regex) is escaped into one `RegexSet` for short-circuit "any match" testing; positions resolved per-pattern only on hits; ranges merged so overlapping matches collapse to one marker. `CompiledFilter` is built per-request for now — the same compilation cost the spec's "RegexSet for speed" line presupposes, and the bench below shows it's cheap.
- Persistence wired in [crates/proxy/src/adapters/google_drive.rs](../../crates/proxy/src/adapters/google_drive.rs): on a Layer-B `Block` decision → `blocked_actions` row with `layer = 'policy'`; on `PicInvariantViolation` from the executor → `layer = 'pic_invariant'`; on `BlockRequest` read filter → `layer = 'read_filter'`; on `ReplaceWithMarker`/`StripSilently` matches → one `quarantined_payloads` row per match.
...
**Status:** Done. **Delivered:**
- [crates/proxy/src/pic/verifier.rs](../../crates/proxy/src/pic/verifier.rs) — `PicVerifier::verify_chain(leaf_pca_id)` walks `pca_cache` from leaf to PCA_0 checking five invariants per hop: (1) CAT signature on each `SignedPca`, (2) continuity (`child.provenance.cat_sig == parent_signed.signature()`), (3) hop ordering, (4) `p_0` immutability, (5) ops monotonicity (`child.ops ⊆ parent.ops`). 60s moka cache by leaf id. The pure invariant checker (`check_invariants`) is split out for unit-testing.
- [crates/proxy/src/api/mod.rs](../../crates/proxy/src/api/mod.rs) — `GET /api/v1/pca/{id}` returns a JSON view + CBOR hex; `GET /api/v1/pca/{id}/verify` returns `{ intact, links_verified, p_0, broken_at, reason }`. Mounted unauthenticated for M1 alongside the existing routes (operator network is the trust boundary; auth lands in §1.6).
...
1. ~~**Live wiremock'd Google integration test deferred.**~~ **Write-gate landed (2026-06-11).** [crates/proxy/src/adapters/google_calendar.rs](../../crates/proxy/src/adapters/google_calendar.rs) `db_backed_calendar_insert_external_attendee_is_blocked_403` drives the Calendar adapter's distinguishing **write** path against real SQL on the shared harness: the §9 `calendar-external-attendee-gate` blocks `events.insert` when an attendee is external → Layer-B `PolicyBlocked` (403) carrying `policy_id` + `override_allowed` + a `layer='policy'` blocked_actions row, with no Trust Plane / Google contacted (the gate decides on `body.external_attendee`). This completes integration coverage of all three adapters (Drive / Gmail / Calendar). Only the *live* happy-path against a real Google Workspace account — verifying our request shaping matches Google's actual Calendar API wire format — remains a manual check (un-mockable by definition).
