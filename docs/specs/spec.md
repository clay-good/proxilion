...
2. **`quarantined_payloads` table not yet created.** The read filter returns `samples` + `matches` in `FilterOutcome` and the action stream records `quarantined_count`, but the per-match audit row the spec mentions lives in a future migration. §1.4 is the natural home.
3. **`blocked_actions` persistence not wired.** Spec mentions persisting a row on `PicInvariantViolation`; for now the rejection lives in `tracing::warn!` + the response. Add when the dashboard's blocked-actions feed lands (§1.6).
